...
def predict():
    ...
    # Sanitize the full file path and ensure it's safe for saving
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], safe_filename_value)
    file_path = safe_file_path(file_path)

    try:
        # Save the file temporarily to the UPLOAD_FOLDER
        file.save(file_path)
    except UnicodeEncodeError as e:
        return jsonify({'error': f"Error saving file: {e}"}), 400

    # Extract features from the audio file and make a prediction
    ...
    if features is not None:
        ...
        predicted_genre = encoder.inverse_transform([np.argmax(prediction)])  # Decode prediction
